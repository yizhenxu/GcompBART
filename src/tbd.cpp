...
//std X, di, ncov
bool bdF(std::vector<std::vector<double> >& X, size_t ncov, tree& x, xinfo& xi, dinfo& di, pinfo& pi, size_t minobsnode, std::vector<double>& binaryX,
        std::vector<double>& L1,std::vector<double>& L2,std::vector<double>& L3,std::vector<double>& L4,
        std::vector<double>& L5,std::vector<double>& L6,std::vector<double>& L7,std::vector<double>& L8)
{
  //GetRNGstate();
  ...
  if(u < PBx) { //do birth
    ...
    //compute sufficient statistics
    sinfo sl,sr; //sl for left from nx and sr for right from nx (using rule (v,c))
    getsuff(X, ncov, x,nx,v,c,xi,di,sl,sr);
    //--------------------------------------------------
    //compute alpha
    ...
    //compute sufficient statistics
    sinfo sl,sr; //sl for left from nx and sr for right from nx (using rule (v,c))
    getsuff(X, ncov, x,nx->getl(),nx->getr(),xi,di,sl,sr);
    //--------------------------------------------------
    //compute alpha
    ...
    double XLogPi, XLogL, YLogPi, YLogL;
    tree::npv nxbnv, nxstarbnv;
    YLogL = lilT1(X, ncov, xstar, nxstar, xi, pi, di, nxstarbnv, 1); // update bottom mu’s of the swapped tree copy
    //is there empty bottom node(s)?
    if(YLogL == 10086.0){ //empty bn from swap, invalid
      xstar.tonull();
      nxstar = 0;
    ...
    XLogPi = lpiT(nx, xi, pi, binaryX);
    XLogL = lilT1(X, ncov, x, nx, xi, pi, di, nxbnv, 0); //do not update bottom mu’s
    ...
    double XLogPi, XLogL, YLogPi, YLogL;
    tree::npv nxbnv, nxstarbnv;
    YLogL = lilT1(X, ncov, xstar, nxstar, xi, pi, di, nxstarbnv, 1); // update bottom mu’s of the changed tree copy
    //is there empty bottom node(s)?
    if(YLogL == 10086.0){ //empty bn from rule change, invalid
      xstar.tonull();
      nxstar = 0;
...
//std X, di, ncov
bool bd1F(std::vector<std::vector<double> >& X, size_t ncov, tree& x, xinfo& xi, dinfo& di, pinfo& pi, size_t minobsnode, std::vector<double>& binaryX,
