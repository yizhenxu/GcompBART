...
// [[Rcpp::export]]
List mybartmod(NumericMatrix XMat,
               double sigest,
               int nn, NumericVector y,
               int n_cov, int nu, double lambda,
               int nd,
               int burn,
               int numtrees,
               double kfac,
  ...
  for(int loop=0;loop < tnd;loop++) { /* Start posterior draws */
    ...
    pi.sigma = sqrt(nlpost/R::rchisq((double)nupost));
    ...
    if(loop>=burn){
      ...
      sigmasample[sigdrawcounter++] = pi.sigma;
  ...
  std::vector<int> yy; 
  yy.resize(2);
  yy.push_back(4);yy.push_back(5);
  
  x.resize(5,99);
  for (size_t i = 0; i < x.size(); ++i)
    std::cout << x[i] << endl;
  x.insert(x.end(),yy.begin(),yy.end());
  */
